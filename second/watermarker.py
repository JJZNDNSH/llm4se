...
from PyQt5.QtGui import QPixmap, QImage, QDragEnterEvent, QDropEvent, QFontDatabase, QCursor,QIcon
...
class Watermarker(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle('批量图片打水印工具 - 中文界面（支持水印拖拽）')
        ...
        self.preview_pixmap = None  # 当前预览的QPixmap（用于鼠标位置计算）
        self.preview_scale_ratio = 1.0  # 预览图与原图的缩放比例（关键：转换鼠标坐标用）

        # 水印拖拽相关状态
        self.is_dragging = False  # 是否正在拖拽水印
        self.drag_start_pos = QPoint()  # 拖拽起始鼠标位置（预览区坐标）
        self.watermark_original_offset = (0, 0)  # 拖拽开始时的水印原始偏移量

        # 水印参数（新增offset默认值，用于拖拽偏移）
        self.template = {
        ...
        self.preview_label.setCursor(QCursor(Qt.ArrowCursor))  # 默认鼠标样式
        ...
        self.size_spin.setValue(self.template['font_size'])
        self.opacity_slider = QSlider(Qt.Horizontal);
        self.opacity_slider.setRange(0, 100);
        self.opacity_slider.setValue(self.template['opacity'])
        self.color_btn = QPushButton('选择颜色')
        self.color_btn.clicked.connect(self.choose_color)
        self.shadow_cb = QCheckBox('阴影');
        self.shadow_cb.setChecked(self.template['shadow'])
        ...
        self.img_scale_slider = QSlider(Qt.Horizontal);
        self.img_scale_slider.setRange(1, 500);
        self.img_scale_slider.setValue(int(self.template['image_scale'] * 100))
        self.img_opacity_slider = QSlider(Qt.Horizontal);
        self.img_opacity_slider.setRange(0, 100);
        self.img_opacity_slider.setValue(self.template['image_opacity'])
        ...
        self.naming_combo.addItems(['保留原文件名', '添加前缀 (例: wm_)', '添加后缀 (例: _watermarked)'])
        ...
        self.resize_combo.addItems(['不缩放', '按宽度', '按高度', '按百分比'])
        self.resize_input = QSpinBox();
        self.resize_input.setRange(1, 10000);
        self.resize_input.setValue(1000)
        ...
        right_v.addWidget(self.output_folder_btn);
        right_v.addWidget(self.output_folder_label)
        right_v.addWidget(QLabel('文件命名规则：'))
        right_v.addWidget(self.naming_combo);
        right_v.addWidget(self.naming_input)
        right_v.addWidget(QLabel('JPEG 质量'))
        right_v.addWidget(self.jpeg_quality_slider)
        right_v.addWidget(QLabel('导出缩放'))
        right_v.addWidget(self.resize_combo);
        right_v.addWidget(self.resize_input)
        right_v.addWidget(self.override_original_cb)
        right_v.addWidget(btn_export)
        right_v.addWidget(btn_save_template);
        right_v.addWidget(btn_load_template)

        right_w = QWidget();
        right_w.setLayout(right_v)

        center_v = QVBoxLayout();
        center_v.addWidget(self.preview_label)
        center_w = QWidget();
        center_w.setLayout(center_v)

        main_h = QHBoxLayout();
        main_h.addWidget(left_w, 2);
        main_h.addWidget(center_w, 5);
        main_h.addWidget(right_w, 3)
    ...
    def import_files_dialog(self):
        ...
        files, _ = QFileDialog.getOpenFileNames(self, '选择图片文件或按取消选择文件夹', '',
    ...
    def clear_list(self):
        ...
        self.template['offset'] = (0, 0)  # 清空列表时重置偏移量
    ...
    def set_position(self, key):
        ...
        self.template['offset'] = (0, 0)  # 点击九宫格时重置偏移量
        self.update_preview()
    ...
    def _gather_template(self):
        return {
            ...
            'offset': self.template.get('offset', (0, 0)),  # 保存偏移量
    ...
    def _apply_template(self, data):
        ...
        self.template['position'] = data.get('position', 'center')
        self.template['offset'] = data.get('offset', (0, 0))  # 加载偏移量
    ...
    def on_preview_mouse_press(self, event):
        """鼠标按下时判断是否点击了水印区域"""
        if not self.current_image or not self.preview_pixmap:
            return
        ...
        if event.button() == Qt.LeftButton:
            ...
            # 检查鼠标是否在水印区域内（简化处理：直接开始拖拽）
            self.is_dragging = True
            self.drag_start_pos = mouse_pos
            self.watermark_original_offset = self.template['offset']  # 保存当前偏移量
            self.preview_label.setCursor(QCursor(Qt.ClosedHandCursor))  # 改变鼠标样式

    def on_preview_mouse_move(self, event):
        """鼠标移动时更新水印位置"""
        if self.is_dragging and self.current_image and self.preview_pixmap:
            # 计算鼠标移动的距离（相对于预览区）
            delta = event.pos() - self.drag_start_pos

            # 将预览区的像素偏移转换为原图的像素偏移（关键：考虑缩放比例）
            scaled_delta_x = int(delta.x() / self.preview_scale_ratio)
            scaled_delta_y = int(delta.y() / self.preview_scale_ratio)

            # 更新水印偏移量
            new_offset = (
                self.watermark_original_offset[0] + scaled_delta_x,
                self.watermark_original_offset[1] + scaled_delta_y
            )
            self.template['offset'] = new_offset
    ...
    def on_preview_mouse_release(self, event):
        ...
        if event.button() == Qt.LeftButton and self.is_dragging:
            ...
            self.preview_label.setCursor(QCursor(Qt.ArrowCursor))  # 恢复鼠标样式
    ...
    def apply_watermark_to_pil(self, base_im: Image.Image) -> Image.Image:
        ...
        offsets = self.template.get('offset', (0, 0))  # 使用拖拽产生的偏移量
        x = 0;
        y = 0
        margin = 10
        if pos_key == 'top-left':
            ...
            x, y = (w - txt_w) // 2, margin
        elif pos_key == 'top-right':
            x, y = w - txt_w - margin, margin
        elif pos_key == 'left':
            x, y = margin, (h - txt_h) // 2
        elif pos_key == 'center':
            x, y = (w - txt_w) // 2, (h - txt_h) // 2
        elif pos_key == 'right':
            x, y = w - txt_w - margin, (h - txt_h) // 2
        elif pos_key == 'bottom-left':
            x, y = margin, h - txt_h - margin
        elif pos_key == 'bottom':
            x, y = (w - txt_w) // 2, h - txt_h - margin
        elif pos_key == 'bottom-right':
            x, y = w - txt_w - margin, h - txt_h - margin
        x += offsets[0];
        y += offsets[1]  # 应用偏移量
        ...
        if self.template.get('image_watermark'):
            try:
                ...
                if pos_key == 'top-left':
                    ...
                    wx, wy = (w - new_w) // 2, margin
                elif pos_key == 'top-right':
                    wx, wy = w - new_w - margin, margin
                elif pos_key == 'left':
                    wx, wy = margin, (h - new_h) // 2
                elif pos_key == 'center':
                    wx, wy = (w - new_w) // 2, (h - new_h) // 2
                elif pos_key == 'right':
                    wx, wy = w - new_w - margin, (h - new_h) // 2
                elif pos_key == 'bottom-left':
                    wx, wy = margin, h - new_h - margin
                elif pos_key == 'bottom':
                    wx, wy = (w - new_w) // 2, h - new_h - margin
                elif pos_key == 'bottom-right':
                    wx, wy = w - new_w - margin, h - new_h - margin
                wx += offsets[0];
                wy += offsets[1]  # 应用偏移量
    ...
    def update_preview(self):
        ...
        oh = out.height;
        ow = out.width
        self.preview_scale_ratio = min(label_w / ow, label_h / oh, 1)  # 保存缩放比例
        disp = out.copy()
        if self.preview_scale_ratio < 1:
            disp = out.resize(
                (int(ow * self.preview_scale_ratio), int(oh * self.preview_scale_ratio)),
                Image.LANCZOS
            )

        self.preview_pixmap = pil_image_to_qpixmap(disp)
    ...
    def export_images(self):
        ...
        for p in self.image_paths:
            if os.path.dirname(p) == self.output_folder and not self.override_original_cb.isChecked():
                QMessageBox.warning(self, '导出目录设置错误',
                                    '导出目录与源图片目录相同，默认禁止覆盖。若确实需要，请勾选允许导出到原文件夹。')
                return
